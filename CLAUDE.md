...
Never edit the deployed copies under `~/.config/...` directly — the next `chezmoi apply` overwrites them. Always edit the source in this repo (or use `chezmoi edit <target>`). The shell starts in `dot_config/`, so editing `dot_config/home-manager/*.nix` here *is* editing the chezmoi source of `~/.config/home-manager/*.nix`.
...
- `.chezmoiignore` excludes the age key and Nix build artifacts/direnv. `private.nix` is **generated** from `private.nix.tmpl` (not ignored). macOS defaults / Touch ID sudo are commented templates in `darwin.nix`. The ignore comments also document Claude skills under `~/.claude/skills/` managed elsewhere (APM / `mizchi/skills`).
